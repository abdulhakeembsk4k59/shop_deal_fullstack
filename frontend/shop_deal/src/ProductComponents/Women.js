...
const Women = () => {
  ...
  const fetchCategoryById = async (categoryName) => {
    try {
      ...
      // Fetch Men's category_id from the backend
      const womenCategoryResponse = await axios.get('http://localhost:4000/get_category_by_name/women');
      if (womenCategoryResponse.data && womenCategoryResponse.data.category && womenCategoryResponse.data.category._id) {
        womenCategoryId = womenCategoryResponse.data.category._id;
      } else {
        throw new Error('Invalid data structure received from the server.');
      }

      // If a parameter is present, fetch its category_id
      let paramCategoryId = womenCategoryId;
      if (categoryName) {
      ...
      fetchMenProductsByCategories(womenCategoryId, paramCategoryId);
    } catch (error) {
  ...
  const fetchMenProductsByCategories = async (categoryId1, categoryId2) => {
    try {
      ...
      console.error('Error fetching Women\'s products:', error.message);

      Swal.fire({
        icon: 'error',
        title: 'Error Fetching Women\'s Products',
        text: error.message || 'Something went wrong!',
      });
    }
  };
  ...
  return (
    <div>
      <Header />
      <h2 className="px-5 pt-2 mb-3">Women {selectedCategory}'s</h2>
